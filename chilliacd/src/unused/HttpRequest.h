...
public:
	HttpRequest(const std::string & sessionId, http_method method, const std::string &uri, const std::string & data = "", int timeout = 0) :m_status(0), m_method(method), m_uri(uri)
		, m_data(data), m_timeout(timeout), evcon(NULL), http_uri(NULL), base(NULL), m_sessionId(sessionId), m_requestTimes(0)
	{
	...
	int Send(struct event_base * _base = NULL){
		...
		LOG4CPLUS_TRACE(log, m_sessionId << " " << "add header Connection:close");
		...
		if (r != 0) {
			LOG4CPLUS_ERROR(log, m_sessionId << " " << "evhttp_make_request() failed");
			m_status = -6;
			goto cleanup;
		}
